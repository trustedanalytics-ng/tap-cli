...
deps_update_tap: verify_gopath
	...
	$(GOBIN)/govendor remove github.com/trustedanalytics-ng/tap-cli/...
	@echo "Done"
...
mock_update:
	$(GOPATH)/bin/mockgen -source=vendor/github.com/trustedanalytics-ng/tap-api-service/client/client.go -package=api -destination=api/api_service_client_mock.go
	$(GOPATH)/bin/mockgen -source=vendor/github.com/trustedanalytics-ng/tap-api-service/client/login.go -package=api -destination=api/api_service_login_mock.go
	./add_license.sh
...
prepare_temp:
	mkdir -p temp/src/github.com/trustedanalytics-ng/tap-cli
	$(eval REPOFILES=$(shell pwd)/*)
	ln -sf $(REPOFILES) temp/src/github.com/trustedanalytics-ng/tap-cli
	mkdir -p application
...
define build
	...
	$(eval APP_DIR_LIST=$(shell GOPATH=$(GOPATH) go list ./temp/src/github.com/trustedanalytics-ng/tap-cli/... | grep -v /vendor/))
